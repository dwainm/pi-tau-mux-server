...
export class SessionSidebar {
  ...
  render() {
    ...
    for (const project of this.projects) {
      ...
      // Use tmux session name if available
      const displayName = project.displayName || project.path;
      const pathDisplay = project.isTmuxSession ? 
        `<span class="tmux-session-name" title="Tmux session">${this.escapeHtml(displayName)}</span>` :
        `<span title="${this.escapeHtml(project.path)}">${this.escapeHtml(displayName)}</span>`;

      header.innerHTML = `
        <span class="chevron">▼</span>
        ${pathDisplay}
        <span class="project-count">${project.sessions.length}</span>
        ${project.isTmuxSession ? '<span class="live-indicator">● LIVE</span>' : ''}
      `;
